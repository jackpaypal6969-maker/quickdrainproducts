...
def _inline(text: str) -> str:
    # Links first, so asterisks/underscores inside a URL are never treated as emphasis.
    parts: list[str] = []
    pos = 0
    for m in _LINK_RE.finditer(text):
        ...
        parts.append(f'<a href="{url}"{rel}>{_format(label)}</a>')
        pos = m.end()
...
def render(source: str) -> Markup:
    ...
    for block in blocks:
        ...
        if re.match(r"^#{1,3}\s", first) and len(lines) == 1:
            hashes = first.split(None, 1)[0]
            title = first[len(hashes):].strip()
            level = min(len(hashes), 3)
            out.append(f"<h{level + 1}>{_inline(title)}</h{level + 1}>" if title else "")
